...
export default function CategoryPage() {
  ...
  const [gamesData, setGamesData] = useState<GamesResponse>({ 
    ...
    totalPages: 1, 
    currentPage: 1,
    category: { id: 0, name: '', category_pilot: '' } 
  });
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    setIsLoading(true);
    fetch(`/api/categories/${categoryId}/games?page=${currentPage}`)
      .then(res => res.json())
            ...
            <div className="flex-1 ml-8">
              ...
              <div className="mt-24 mb-4">
                <div className="container mx-auto px-4">
                  {!isLoading && gamesData.category && (
                    <h1 className="text-2xl font-bold text-white">
                      {gamesData.category.name}
                    </h1>
                  )}
                </div>
              </div>
                ...
                <div className="container mx-auto px-4">
                  ...
                  {gamesData.games.length > 0 && (
                    <div className="mt-8 mb-8 flex justify-center items-center gap-2">
                      <Link
                        href={currentPage === 1 ? `/category/${categoryId}` : `/category/${categoryId}/${Math.max(1, currentPage - 1)}`}
                      ...
                      <div className="flex gap-2">
                        {generatePaginationButtons().map((page, index) => (
                          page === '...' ? (
                            ...
                            <Link
                              key={`page-${page}`}
                              href={page === 1 ? `/category/${categoryId}` : `/category/${categoryId}/${page}`}
                      ...
                      <Link
                        href={`/category/${categoryId}/${Math.min(gamesData.totalPages, currentPage + 1)}`}
                        className={`px-4 py-2 rounded transition-colors ${
